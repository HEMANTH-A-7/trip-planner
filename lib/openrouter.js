...
async function postToOpenRouter(messages, abortSignal) {
  ...
  const res = await fetch(OPENROUTER_URL, {
    ...
    body: JSON.stringify({
      ...
      // This is a fallback that runs *after* the primary has already failed,
      // against the clock of the shared request budget. The free models
      // default to a long reasoning pass that routinely blows that budget;
      // the schema is spelled out in the prompt, so turn reasoning off and
      // let the model answer directly.
