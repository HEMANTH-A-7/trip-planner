...
// gemini-3.x flash is a reasoning model and thinks for a long time by default
// - enough to blow the request timeout on a "refine" call. "LOW" keeps enough
// reasoning for a coherent itinerary while cutting the latency roughly in
// half. Set GEMINI_THINKING_LEVEL to override (MINIMAL | LOW | MEDIUM | HIGH).
...
function getClient() {
  ...
  // Cap the SDK's retry loop (default is 5 attempts with backoff). On a 503
  // "model overloaded" that default burns ~40s retrying a model that is down,
  // leaving nothing in the request budget for the OpenRouter fallback. One
  // retry is enough for a transient blip; a real outage should fail fast and
