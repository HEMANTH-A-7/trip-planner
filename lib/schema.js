...
// Adds client-side ids so the UI has stable keys to expand/remove/reorder
// against. Called once right after an itinerary passes validation, whether
// it's a brand-new itinerary or the result of a refinement request.
...
// Strips the client-only ids back out before sending the current itinerary
// to Gemini as context for a refinement request — the model only needs the
// content, not our bookkeeping fields.
export function stripIds(itinerary) {
  return {
    ...itinerary,
    days: itinerary.days.map(({ id: _id, stops, ...day }) => ({
      ...day,
