...
export const DaySchema = z.object({
  ...
  //
  // 120 rather than 80 because the model is asked for "the item, and what
  // it's for" on one line, which 80 characters routinely can't hold. The cap
  // is a guard against a runaway response, not a style rule - and it has to
  // sit above what the prompt actually asks for, or a checklist line three
  // words too long would fail the whole itinerary as bad_shape. Raising it is
  // backward compatible: everything already saved is well under it.
  packingChecklist: z.array(z.string().max(120)).optional(),
});
...
let nextId = 0;
// Exported so a stop the traveler adds by hand is minted from the same
// counter as the ones that arrive from the model - two id sources would
// eventually agree on a value and collide as React keys.
